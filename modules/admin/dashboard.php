<?php
    ...
    ");
            ...
            try { $pdo->query("SELECT 1");
                $healthStatus['Main DB'] = ['status' => 'ok', 'note' => ''];} catch (Throwable $e) {$healthStatus['Main DB'] = ['status' => 'down', 'note' => 'Koneksi gagal'];}

            if ($pdo_bot) {
                try { $pdo_bot->query("SELECT 1");
                    $healthStatus['Bot DB'] = ['status' => 'ok', 'note' => ''];} catch (Throwable $e) {$healthStatus['Bot DB'] = ['status' => 'down', 'note' => 'Koneksi gagal'];}
            } else {
                $healthStatus['Bot DB'] = ['status' => 'warn', 'note' => 'Tidak terkonfigurasi'];
            }

            try { $pdo_run->query("SELECT 1");
                $healthStatus['Run DB'] = ['status' => 'ok', 'note' => ''];} catch (Throwable $e) {$healthStatus['Run DB'] = ['status' => 'down', 'note' => 'Koneksi gagal'];}

            if ($pdo_war) {
                try { $pdo_war->query("SELECT 1");
                    $healthStatus['War DB'] = ['status' => 'ok', 'note' => ''];} catch (Throwable $e) {$healthStatus['War DB'] = ['status' => 'down', 'note' => 'Koneksi gagal'];}
            } else {
            ...
            try {
                ...
                if (! empty($ftp_config_check['host'])) {
                    ...
                    if ($conn) {
                        ...
                        if ($login) {$healthStatus['FTP Connection'] = ['status' => 'ok', 'note' => 'Terhubung'];@ftp_close($conn);} else { $healthStatus['FTP Connection'] = ['status' => 'warn', 'note' => 'Login gagal'];@ftp_close($conn);}
                    } else { $healthStatus['FTP Connection'] = ['status' => 'down', 'note' => 'Tidak dapat terhubung'];}
                } else { $healthStatus['FTP Connection'] = ['status' => 'warn', 'note' => 'Tidak terkonfigurasi'];}
            } catch (Throwable $e) {$healthStatus['FTP Connection'] = ['status' => 'down', 'note' => 'Error: ' . $e->getMessage()];}
            ...
            try {
                ...
                if (is_dir($storagePath)) {
                    ...
                    } else { $healthStatus['Storage / Log Size'] = ['status' => 'unknown', 'note' => 'Tidak dapat dibaca'];}
                } else { $healthStatus['Storage / Log Size'] = ['status' => 'unknown', 'note' => 'Folder storage tidak ditemukan'];}
            } catch (Throwable $e) {$healthStatus['Storage / Log Size'] = ['status' => 'unknown', 'note' => 'Error'];}
            ...
            try {
                ...
                if (file_exists($errorLog) && is_readable($errorLog)) {
                    ...
                    } else { $healthStatus['Recent PHP Errors'] = ['status' => 'ok', 'note' => 'Tidak terbaca'];}
                } else { $healthStatus['Recent PHP Errors'] = ['status' => 'ok', 'note' => 'Tidak ada log'];}
            } catch (Throwable $e) {$healthStatus['Recent PHP Errors'] = ['status' => 'unknown', 'note' => 'Error membaca log'];}
