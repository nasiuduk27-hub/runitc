...
foreach ($completed_takers as $acc) {
    ...
    foreach ($ans_by_type as $m_lrtype => $ans_list) {
        ...
        $lseljawaban .= Encrypt($ljawaban, $lkeylock);
    }
     ...
     */
    $authorcryp = fw(Encrypt($authorize, $lkeylock), 10);
    $idno       = fw($acc['idno'] ?? '', 20);
    ...
    $questioner = fw(Encrypt($acc['questioner'] ?? '', $lkeylock), 36);
    ...
    $crc .= fw($admin_code, 6);
    $crc .= $authorcryp;
    $crc .= fw(Encrypt($testdt_str, $lkeylock), 8);
    $crc .= $idno;
    $crc .= $regnm;
    $crc .= $mdob;
    $crc .= $countcd;
    ...
    $crc .= $macno;
    $crc .= $cmpnm;
    $crc .= fw(Encrypt($lselparam, $lkeylock), 13);
