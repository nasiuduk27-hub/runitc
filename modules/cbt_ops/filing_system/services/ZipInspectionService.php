<?php
// File: modules/cbt_ops/filing_system/services/ZipInspectionService.php
...
class ZipInspectionService
{
    ...
    public function hasPathTraversal(string $filename): bool
    {
        ...
        if (strpos($filename, '..\\') !== false) return true;
        if (strpos($filename, '/') === 0) return true;
        if (strpos($filename, '\\') === 0) return true;
        if (preg_match('/^[a-zA-Z]:\\\\/', $filename)) return true;
        return false;
    }
    ...
    public function inspect(string $zipPath): array
    {
        ...
        for ($i = 0; $i < $scanLimit; $i++) {
            ...
            if ($this->hasPathTraversal($name)) {
                ...
                if (!in_array('Path Traversal Detected', $globalWarnings)) $globalWarnings[] = 'Path Traversal Detected';
            }

            if ($this->isDangerousExtension($ext)) {
                $fileWarnings[] = 'Dangerous Extension';
                if (!in_array('Dangerous Extension Detected', $globalWarnings)) $globalWarnings[] = 'Dangerous Extension Detected';
            }
