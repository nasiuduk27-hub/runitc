...
<div class="fs-scrollbar min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-4 md:p-6 flex flex-col gap-5">            <input type="hidden" id="perm_filing_id">
            ...
            <!-- Access Mode -->
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Mode Akses Utama</label>
                <select id="perm_access_mode" class="fs-select w-full md:w-1/2 px-4 py-2 text-sm border border-gray-300 rounded focus:ring-yellow-500 focus:border-yellow-500 bg-white">
                    <option value="private">Private (Hanya Owner & Rule Spesifik)</option>
                    <option value="public_internal">Public Internal (Semua User Login)</option>
                    <option value="custom">Custom (User / Role / Company / Group)</option>
                    <option value="share_link">Share Link (Hanya via Tautan)</option>
                </select>
                <p class="text-xs text-gray-500 mt-1">Mengubah mode akses utama dapat mempengaruhi visibilitas file bagi pengguna yang tidak memiliki aturan spesifik.</p>
            </div>
...
function renderRulesTable() {
    ...
    if (currentRules.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="px-4 py-6 text-center text-gray-400 italic text-xs">Belum ada aturan spesifik. Akses mengikuti Mode Akses Utama.</td></tr>';
        return;
    }
