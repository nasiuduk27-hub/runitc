...
class FilingExpiryService
{
    ...
    private ?FileSystemDrive $drive = null;
    ...
    /**
     * Nama tabel aktif: file_system (tabel baru) atau sys_filing (legacy).
     */
    private function table(): string
    {
        return $this->usesNewTables() ? 'file_system' : 'sys_filing';
    }

    private function drive(): FileSystemDrive
    {
        return $this->drive ??= new FileSystemDrive($this->db);
    }

    /**
     * Path fisik file sesuai tabel aktif.
     */
    private function storagePathFor(array $file): string
    {
        if ($this->usesNewTables()) {
            return $this->drive()->buildStoragePath($file);
        }
        ...
        foreach ($files as $file) {
            ...
            if ($result['success']) {
                ...
                switch ($result['status']) {
                    case 'archived': $stats['total_archived']++;
                        break;
                    case 'trashed': $stats['total_trashed']++;
                        break;
                    case 'expired': $stats['total_expired']++;
                        break;
                }
            } else {
    ...
    public function purgeOldTrash(int $retentionDays = 30, int $limit = 100): array
    {
        ...
        $table = $this->table();
        $select = $this->usesNewTables() ? '*' : 'rec_id, storage_path';
        ...
        foreach ($files as $file) {
            try {
                ...
                if ($this->storage !== null) {
                    $storagePath = $this->storagePathFor($file);
                    if ($storagePath !== '') {
                        $remoteSize = $this->storage->getFileSize($storagePath);
                        $deletedPhysically = $remoteSize <= 0 || $this->storage->deletePhysicalFile($storagePath);
                    }
                }
                ...
                $up = $this->db->prepare("UPDATE {$table} SET status = 'deleted', deleted_at = NOW(), updated_at = NOW() WHERE rec_id = ? AND status = 'trashed'");
