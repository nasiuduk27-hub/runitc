...
class FilingStorageService
{
    ...
    private function downloadFromPublicUrl(string $storagePath, string $localPath): bool
    {
        ...
        $baseUrl = '';
        if (str_starts_with($path, 'filesys/')) {
            // Skema baru (file_system): file berada di server storage public (fsys).
            $baseUrl = trim((string) (getenv('FILING_PUBLIC_BASE_URL') ?: ''));
            if ($baseUrl === '') {
                return false;
            }
        } else {
    ...
    private function resolveRemotePath(string $storagePath): string
    {
        ...
        // Format baru (file_system): path penuh dari sys_msttable.notes + upload_flnm,
        // contoh /filesys/cid26b/FSY-25L-00017.pdf -> filesys/cid26b/FSY-25L-00017.pdf
        if (str_starts_with($path, 'filesys/')) {
            return $path;
        }
