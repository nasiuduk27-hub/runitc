...
class FilingSystemRecord
{
    ...
    public function getAllAssignedAdmins(): array
    {
        [$yearStart, $nextYearStart] = $this->currentYearRange();
    ...
    public function getFilteredFilingData(array $filters, array $adminClientMap, ?array $spvFilter = null): array
    {
        ...
        $filingIds = array_map('intval', array_column($rows, 'rec_id'));
        $issueCounts = $this->getIssueCountsByFilingIds($filingIds);
        $filesByFilingId = $this->getFilesByFilingIds($filingIds);
        $participantCounts = $this->getParticipantCountsByRows($rows);
        ...
        foreach ($rows as $row) {
            ...
            $row['issues_count'] = $issueCounts[(int) $row['rec_id']] ?? 0;
            $row['files'] = $filesByFilingId[(int) $row['rec_id']] ?? [];
            $adminId = (int) ($row['admin_id'] ?? 0);
            $subAdminId = (string) ($row['sub_admin_id'] ?? '');
            $counts = $adminId > 0
                ? ($participantCounts['by_admin'][$adminId] ?? null)
                : ($subAdminId !== '' ? ($participantCounts['by_sub_admin'][$subAdminId] ?? null) : null);
            $row['total_count'] = $counts['total_count'] ?? 0;
            $row['finished_count'] = $counts['finished_count'] ?? 0;
    ...
    public function getFilingDataFromAssignments(array $filters, array $adminClientMap, ?array $spvFilter = null): array
    {
        $assignments = $spvFilter !== null
            ? $this->assignmentRowsForSpvFilter($spvFilter)
            : $this->getAllAssignedAdmins();

        $search = trim((string) ($filters['search'] ?? ''));
        $adminFilter = trim((string) ($filters['admin'] ?? ''));
        $clientFilter = trim((string) ($filters['client'] ?? ''));
        ...
        foreach ($assignments as $assignment) {
            ...
            $testDate = ! empty($assignment['testdt']) ? date('Y-m-d', strtotime((string) $assignment['testdt'])) : date('Y-m-d');
            $clientName = $adminClientMap[$adminNo] ?? '-';
            $spvName = trim((string) ($assignment['spv_name'] ?? ($spvNames[(int) ($assignment['spv_recid'] ?? 0)] ?? '-')));

            if (! isset($grouped[$adminNo])) {
                $grouped[$adminNo] = [
                    'nomor_admin' => $adminNo,
                    'sub_admin_id' => (string) ($assignment['sub_admin_id'] ?? ''),
                    'admin_id' => (int) ($assignment['admin_id'] ?? 0),
                    'tanggal' => $testDate,
                    'client_name' => $clientName,
                    'spv_name' => $spvName !== '' ? $spvName : '-',
                    'total_count' => 0,
                ];
            }

            $grouped[$adminNo]['total_count'] += (int) ($assignment['authorize_amt'] ?? 0);
            $grouped[$adminNo]['admin_id'] = (int) ($assignment['admin_id'] ?? 0);
        }

        foreach ($grouped as $adminNo => $g) {
            $spvName = $g['spv_name'];
            if ($search !== '' && stripos($adminNo, $search) === false && stripos($spvName, $search) === false) {
                continue;
            }
            if ($adminFilter !== '' && $adminNo !== $adminFilter) {
                continue;
            }
            if ($clientFilter !== '' && $g['client_name'] !== $clientFilter) {
                continue;
            }
            if ($dateFilter !== '' && ! $this->dateMatchesFilter($g['tanggal'], $dateFilter)) {
                continue;
            }
            ...
            $data[] = [
                'rec_id' => 0,
                'nomor_admin' => $adminNo,
                'sub_admin_id' => $g['sub_admin_id'],
                'admin_id' => $g['admin_id'],
                'tanggal' => $g['tanggal'],
                'keterangan' => '',
                'input_by' => 'System Assignment',
                'spv_name' => $spvName !== '' ? $spvName : '-',
    ...
    private function assignmentRowsForSpvFilter(array $spvFilter): array
    {
        $assignedPairs = $spvFilter['assigned_admin_pairs'] ?? [];
        if (empty($assignedPairs)) {
            return [];
        ...
        $all = $this->getAllAssignedAdmins();
        $keys = [];
        foreach ($assignedPairs as $pair) {
            $keys[(int) ($pair['admin_id'] ?? 0).'|'.(string) ($pair['sub_admin_id'] ?? '')] = true;
        }

        return array_values(array_filter($all, static function (array $row) use ($keys): bool {
            $key = (int) ($row['admin_id'] ?? 0).'|'.(string) ($row['sub_admin_id'] ?? '');

            return isset($keys[$key]);
        }));
    }

    private function dateMatchesFilter(string $date, string $filter): bool
    {
        if (strpos($filter, ' to ') !== false) {
            [$start, $end] = array_map('trim', explode(' to ', $filter, 2));

            return $date >= $start && $date <= $end;
        }

        return $date === trim($filter);
    }

    private function getIssueCountsByFilingIds(array $filingIds): array
    {
        $filingIds = array_values(array_unique(array_filter(array_map('intval', $filingIds))));
        if (empty($filingIds)) {
            return [];
        }

        $placeholders = implode(',', array_fill(0, count($filingIds), '?'));
        $stmt = $this->pdoRun->prepare("
            SELECT filing_id, COUNT(*) AS issue_count
            FROM runit_filing_issues
        ...
        $counts = [];
        foreach ($stmt->fetchAll(\PDO::FETCH_ASSOC) as $row) {
            $counts[(int) $row['filing_id']] = (int) $row['issue_count'];
        }

        return $counts;
    }

    private function getFilesByFilingIds(array $filingIds): array
    {
        $filingIds = array_values(array_unique(array_filter(array_map('intval', $filingIds))));
        if (empty($filingIds)) {
            return [];
        ...
        $placeholders = implode(',', array_fill(0, count($filingIds), '?'));
        $stmt = $this->pdoRun->prepare("
            SELECT file_id, filing_id, file_name, file_path, file_type, file_category, uploaded_at
            FROM runit_filing_files
            WHERE filing_id IN ({$placeholders})
            ORDER BY filing_id ASC, file_id DESC
        ");
        $stmt->execute($filingIds);

        $files = [];
        foreach ($stmt->fetchAll(\PDO::FETCH_ASSOC) as $file) {
            $files[(int) $file['filing_id']][] = $file;
        }

        return $files;
    }

    private function getParticipantCountsByRows(array $rows): array
    {
        $adminIds = [];
        $subAdminIds = [];
        ...
        foreach ($rows as $row) {
            ...
            if ($adminId > 0) {
                ...
                $subAdminIds[] = $subAdminId;
            }
        }

        $counts = ['by_admin' => [], 'by_sub_admin' => []];
        ...
        if (! empty($adminIds)) {
            ...
            $stmt = $this->pdoWar->prepare("
                SELECT
                    ...
                    SUM(CASE WHEN statrec IN ('7','8','9','c','C') THEN 1 ELSE 0 END) AS finished_count
                FROM t3sTt4keR5
                WHERE admin_id IN ({$adminPlaceholders})
                GROUP BY admin_id
            ");
            $stmt->execute($adminIds);

            foreach ($stmt->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                $counts['by_admin'][(int) $row['admin_id']] = [
        ...
        $subAdminIds = array_values(array_unique($subAdminIds));
        if (! empty($subAdminIds)) {
            $subPlaceholders = implode(',', array_fill(0, count($subAdminIds), '?'));
            $stmt = $this->pdoWar->prepare("
                SELECT
                    sub_adm_id,
                    COUNT(*) AS total_count,
                    SUM(CASE WHEN statrec IN ('7','8','9','c','C') THEN 1 ELSE 0 END) AS finished_count
                FROM t3sTt4keR5
                WHERE sub_adm_id IN ({$subPlaceholders})
                GROUP BY sub_adm_id
            ");
            $stmt->execute($subAdminIds);

            foreach ($stmt->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                $counts['by_sub_admin'][(string) $row['sub_adm_id']] = [
                    'total_count' => (int) ($row['total_count'] ?? 0),
                    'finished_count' => (int) ($row['finished_count'] ?? 0),
                ];
            }
