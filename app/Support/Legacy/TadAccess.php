...
class TadAccess
{
    public static function getRoleRows(\PDO $pdoRun, int $userId): array
    {
        if ($userId <= 0) {
            return [];
        }
    ...
    public static function userHasRole(\PDO $pdoRun, int $userId, array $allowedRoles): bool
    {
        $roles = self::getRoleRows($pdoRun, $userId);
    ...
    public static function canManageSupervisor(\PDO $pdoRun, int $userId): bool
    {
        return self::userHasRole($pdoRun, $userId, ['TAD ADMIN', 'TAD STAFF', 'SUPER ADMIN']);
    }

    public static function canManageDistribution(\PDO $pdoRun, int $userId): bool
    {
        return self::userHasRole($pdoRun, $userId, ['TAD ADMIN', 'TAD STAFF', 'SUPER ADMIN']);
    }

    public static function getSupervisorIdForUser(\PDO $pdoRun, int $userId): int
    {
        if ($userId <= 0) {
            return 0;
        }
    ...
    public static function ensureUserHasSpvRole(\PDO $pdoRun, int $userId): void
    {
        if ($userId <= 0 || self::userHasRole($pdoRun, $userId, ['SUPER ADMIN'])) {
            return;
        }
