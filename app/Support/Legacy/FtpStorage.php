...
class FtpStorage
{
    ...
    public function modifiedTime(string $remotePath): int
    {
        ...
        $remotePath = $this->normalizePath($remotePath);
        $time = @ftp_mdtm($this->connection, $remotePath);

        return is_int($time) ? $time : -1;
    }
    ...
    public function listFilesWithDiagnostics(string $remoteDir): array
    {
        ...
        if (preg_match('#^CRC_B2(?:/|$)#i', $remoteDir)) {
            $addCandidate('CBT/'.$remoteDir);
            $addCandidate('docs/CBT/'.$remoteDir);
            $addCandidate('cbt.toeic.or.id/docs/CBT/'.$remoteDir);
            $addCandidate('/www/wwwroot/cbt.toeic.or.id/docs/CBT/'.$remoteDir);
            $addCandidate('www/wwwroot/cbt.toeic.or.id/docs/CBT/'.$remoteDir);
        }

        if (preg_match('#^collector/final(?:/|$)#i', $remoteDir)) {
            $addCandidate('docs/'.$remoteDir);
            $addCandidate('cbt.toeic.or.id/docs/'.$remoteDir);
            $addCandidate('/www/wwwroot/cbt.toeic.or.id/docs/'.$remoteDir);
            $addCandidate('www/wwwroot/cbt.toeic.or.id/docs/'.$remoteDir);
        }
    ...
    public function normalizePath(string $remotePath): string
    {
        ...
        if ($basePath !== '') {
            if (strcasecmp($basePath, 'CRC') === 0 && preg_match('#^CRC_B2(?:/|$)#i', $remotePath)) {
