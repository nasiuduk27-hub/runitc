...
class Monitoring
{
    ...
    public function getParticipants($adminId, $subAdminId, string $adminNo, ?string $testCode): array
    {
        ...
        $psyskunci = shifting(trim($adminNo), 3);
    ...
    {
        $namaAsli = deccrypt(trim($row['regnm'] ?? ''), $psyskunci);
        ...
        return [
            ...
            'regnm'    => $namaAsli,
            'idno'     => deccrypt(trim($row['idno'] ?? ''), $psyskunci),
            'nisn'     => deccrypt(trim($row['idno'] ?? ''), $psyskunci),
            'dob'      => deccrypt(trim($row['dob'] ?? ''), $psyskunci),
            'sexmf'    => deccrypt(trim($row['sexmf'] ?? ''), $psyskunci),
            'email'    => deccrypt(trim($row['email'] ?? ''), $psyskunci),
            'hpno'     => deccrypt(trim($row['hpno'] ?? ''), $psyskunci),
            'countcd'  => deccrypt(trim($row['countcd'] ?? ''), $psyskunci),
            'langcode' => deccrypt(trim($row['langcode'] ?? ''), $psyskunci),
            'groupcd'  => deccrypt(trim($row['groupcd'] ?? ''), $psyskunci),
            'custom1'  => deccrypt(trim($row['custom1'] ?? ''), $psyskunci),
            'custom2'  => deccrypt(trim($row['custom2'] ?? ''), $psyskunci),
            'custom3'  => deccrypt(trim($row['custom3'] ?? ''), $psyskunci),
