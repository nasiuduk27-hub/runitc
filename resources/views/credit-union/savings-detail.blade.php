...
<div class="mx-auto max-w-6xl space-y-6">
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        <form method="GET" action="{{ route('cu.savings.detail') }}" class="grid grid-cols-1 gap-3 border-b border-gray-100 px-5 py-4 md:grid-cols-[minmax(0,1fr)_12rem_12rem_auto_auto] md:items-end">
            <label class="block">
            ...
            <label class="block">
                ...
                <x-date-input name="date" id="cu_savings_date" value="{{ $filters['date'] }}" class="w-full rounded-xl border-gray-300 text-sm shadow-sm focus:border-brand-primary focus:ring-brand-primary" />
            </label>
            ...
            <a href="{{ route('cu.savings.detail') }}" class="rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-center text-xs font-semibold text-gray-600 transition hover:border-brand-primary/40 hover:text-brand-primary">Reset</a>
        </form>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($savingsRows as $row)
                        <tr class="text-gray-700">
                            <td class="whitespace-nowrap px-5 py-3">{{ \App\Services\CreditUnion\CreditUnionPeriod::label((string) $row->pprd) }}</td>
                            <td class="px-5 py-3">
