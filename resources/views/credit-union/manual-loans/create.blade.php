...
<div class="mx-auto max-w-7xl space-y-6">
    ...
    @if ($mode !== 'loan')
        <form method="POST" action="{{ route('cu.manual-loans.adjustment') }}" id="adjustmentForm" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><label for="adjustment_start" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Mulai {{ $mode === 'skip' ? 'Skip' : 'Percepatan' }} (YYYYMM) <span class="text-red-500">*</span></label><select id="adjustment_start" name="start_period" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"><option value="">Pilih loan terlebih dahulu</option></select></div><div><label for="adjustment_months" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">{{ $mode === 'skip' ? 'Lama Skip' : 'Percepatan' }} (Bulan) <span class="text-red-500">*</span></label><input id="adjustment_months" name="months_count" type="number" min="1" max="12" value="1" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"></div></div>
    ...
    <form method="POST" action="{{ route('cu.manual-loans.store') }}" id="manualLoanForm" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div><label for="member_rec_id" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Anggota <span class="text-gray-400">(opsional)</span></label><select id="member_rec_id" name="member_rec_id" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"><option value="">Nama manual</option>@foreach ($members as $member)<option value="{{ $member->rec_id }}" data-name="{{ $member->icunm }}" @selected(old('member_rec_id') == $member->rec_id)>{{ $member->icuno }} - {{ $member->icunm }}</option>@endforeach</select><p class="mt-1 text-[11px] text-gray-400">Pilih anggota terdaftar, atau biarkan kosong untuk anggota baru.</p></div>
            <div><label for="member_name" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Nama Anggota <span class="text-red-500">*</span></label><input id="member_name" name="member_name" value="{{ old('member_name') }}" @readonly(old('member_rec_id')) placeholder="contoh: BUDI SANTOSO" required class="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-semibold outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20 {{ old('member_rec_id') ? 'bg-gray-100 text-gray-500' : 'bg-gray-50 text-gray-800' }}"><p class="mt-1 text-[11px] text-gray-400">Terisi & terkunci otomatis saat anggota dipilih. Isi manual untuk anggota baru.</p></div>
            ...
            <div><label for="trndt" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Tanggal Transaksi <span class="text-red-500">*</span></label><x-date-input name="trndt" id="trndt" value="{{ old('trndt', date('Y-m-d')) }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"></div>
            ...
            <div><label for="principal" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Pokok Pinjaman <span class="text-red-500">*</span></label><input id="principal" name="principal" type="text" inputmode="numeric" autocomplete="off" min="1" value="{{ old('principal') }}" placeholder="contoh: 5.000.000" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"></div>
            <div><label for="term" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Tenor (bulan) <span class="text-red-500">*</span></label><input id="term" name="term" type="number" min="1" max="120" value="{{ old('term') }}" placeholder="contoh: 12" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"></div>
            <div><label for="annual_rate" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Bunga Tahunan (%) <span class="text-red-500">*</span></label><input id="annual_rate" name="annual_rate" type="number" min="0" max="100" step="0.01" value="{{ old('annual_rate', 6) }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"></div>
            <div><label for="payment_status" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Status Historis</label><select id="payment_status" name="payment_status" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"><option value="paid">Sudah Lunas</option><option value="running">Masih Berjalan</option></select></div>
            <div><label for="admin_fee" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Biaya Admin Bank (Rp)</label><input id="admin_fee" name="admin_fee" type="text" inputmode="numeric" autocomplete="off" value="{{ old('admin_fee') }}" placeholder="0" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"><p class="mt-1 text-[11px] text-gray-400">Diisi manual sesuai biaya admin bank.</p></div>
            <div><label for="admin_fee_type" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Tipe Biaya Admin</label><select id="admin_fee_type" name="admin_fee_type" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20"><option value="exclude" @selected(old('admin_fee_type', 'exclude') === 'exclude')>Exclude, ditagih ke anggota</option><option value="include" @selected(old('admin_fee_type') === 'include')>Include, dipotong dari pencairan</option></select></div>
            <input type="hidden" name="calculation_method" value="flat">
        </div>
        <div class="flex flex-wrap gap-2"><button type="button" id="simulateButton" class="inline-flex items-center gap-2 rounded-xl border border-brand-primary bg-white px-5 py-2.5 text-sm font-semibold text-brand-primary transition hover:bg-blue-50"><i class="fas fa-calculator"></i> Simulasikan</button><button type="submit" id="saveButton" onclick="return confirm('Simpan loan manual ini?')" disabled class="inline-flex items-center gap-2 rounded-xl bg-brand-primary px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-brand-primary/30 transition hover:bg-brand-primaryHover disabled:cursor-not-allowed disabled:opacity-50"><i class="fas fa-save"></i> Simpan Loan Manual</button><button type="reset" class="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-sm font-semibold text-gray-600 transition hover:bg-gray-50">Batal</button></div>
    </form>
    @endif
...
(function () {
    ...
    dateDisplay.addEventListener('change', () => { updatePeriod(); save.disabled = true; });
