                ...
                <section>
                    ...
                    <div class="fs-card bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <p class="text-[10px] font-bold text-gray-400 uppercase">Share To</p>
                        ...
                        <div class="border-t border-gray-50 pt-4" id="drawer_shared_to_wrap">
                            <p class="text-[10px] font-bold text-gray-400 uppercase mb-2">Dibagikan ke Divisi / Company</p>
                            <div id="drawer_shared_to" class="flex flex-wrap gap-1.5"></div>
                        </div>
                    </div>
                </section>
...
if (!res.can_see_full_info) {
            ...
            document.getElementById('drawer_shared_to_wrap')?.classList.add('hidden');
            return;
        }

        // Dibagikan ke Divisi / Company
        const sharedToWrap = document.getElementById('drawer_shared_to_wrap');
        const sharedToBox = document.getElementById('drawer_shared_to');
        if (sharedToWrap) sharedToWrap.classList.remove('hidden');
        if (sharedToBox) {
            const targets = res.shared_to || [];
            if (targets.length === 0) {
                sharedToBox.innerHTML = '<span class="text-xs text-gray-400 italic">Belum dibagikan ke divisi/company.</span>';
            } else {
                sharedToBox.innerHTML = targets.map(t => `<span class="bg-indigo-50 text-indigo-700 border border-indigo-100 text-[10px] font-bold px-2 py-1 rounded-full">${escapeHtml(t.label)}</span>`).join('');
            }
        }
