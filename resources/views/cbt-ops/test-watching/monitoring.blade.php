...
@else
    ...
    <head>
        ...
        <script>
            tailwind.config = { theme: { extend: { colors: { brand: { primary: '#1D4ED8', primaryHover: '#1E40AF', bg: '#f3f4f6', card: '#ffffff' } } } } };
        </script>
        <style>
            ...
            <?php if ($has_jadwal) { ?>
                ...
                <div class="max-h-64 overflow-y-auto" id="dropdownListContainer">
                    <?php foreach ($filtered_admins as $key => $data) { ?>
                        <?php $itemMode = $monitoringModeFromAdminData($data); ?>
...
<form action="<?php echo route('cbt-ops.test-watching.upload-crc') ?>" method="POST" enctype="multipart/form-data" class="mt-4" id="manualCrcUploadForm">
    ...
    <label class="block mb-2 text-sm font-medium text-gray-900" for="crc_files">Upload file .CRC manual (bulk)</label>
    <div id="crcDropZone" class="border-2 border-dashed border-green-200 bg-green-50/40 rounded-xl p-5 text-center cursor-pointer hover:bg-green-50 transition">
        <input class="hidden" id="crc_files" name="crc_files[]" type="file" accept=".crc,.CRC" multiple required>
        <div class="text-green-700 text-2xl mb-2"><i class="fas fa-file-upload"></i></div>
        <div class="text-sm font-bold text-gray-800">Drop banyak file .CRC di sini</div>
        <div class="text-[10px] text-gray-500 mt-1">atau klik area ini untuk memilih maksimal 500 file sekaligus</div>
        <div id="crcSelectedFiles" class="text-[11px] text-green-700 font-bold mt-3">Belum ada file dipilih.</div>
    </div>
    <p class="text-[10px] text-gray-500 mt-2">Maksimal 500 file .CRC, 1MB per file, total 10MB. Semua file disimpan sebagai CRC Individu ke folder tanggal ujian.</p>
    <button type="submit" id="btnManualCrcUpload" class="w-full mt-4 bg-green-600 hover:bg-green-700 text-white py-2.5 rounded-lg text-sm font-bold transition flex justify-center items-center gap-2">
        <i class="fas fa-cloud-upload-alt"></i> Upload CRC
    </button>
</form>
<script>
    (function () {
        const dropZone = document.getElementById('crcDropZone');
        const input = document.getElementById('crc_files');
        const selected = document.getElementById('crcSelectedFiles');
        const form = document.getElementById('manualCrcUploadForm');
        const btn = document.getElementById('btnManualCrcUpload');
        ...
        function updateSelectedLabel() {
            ...
            selected.textContent = files.length > 0
                ? files.length + ' file .CRC siap diupload: ' + files.map(file => file.name).slice(0, 3).join(', ') + (files.length > 3 ? ', ...' : '')
                : 'Belum ada file dipilih.';
        }

        function setFiles(files) {
            const crcFiles = Array.from(files || []).filter(file => file.name.toLowerCase().endsWith('.crc'));
            const dataTransfer = new DataTransfer();
            crcFiles.forEach(file => dataTransfer.items.add(file));
        ...
        function buildChunkFormData(files) {
            const data = new FormData();
            form.querySelectorAll('input[type="hidden"]').forEach(input => data.append(input.name, input.value));
            files.forEach(file => data.append('crc_files[]', file, file.name));
            return data;
        }

        async function uploadChunk(files, index, total) {
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Uploading batch ' + index + '/' + total + '...';
            const response = await fetch(form.action, {
                method: 'POST',
                body: buildChunkFormData(files),
            ...
            try {
                ...
                if (start >= 0 && end > start) {
                    ...
                    throw new Error('Response server bukan JSON: ' + text.substring(0, 200));
                }
            }

            if (!response.ok || !result.success) {
                throw new Error(result.message || 'Upload CRC gagal pada batch ' + index + '.');
            }
        ...
        if (form && btn) {
            form.addEventListener('submit', async event => {
                ...
                try {
                    ...
                    let failedCount = 0;
                    const chunks = [];
                    for (let i = 0; i < files.length; i += uploadChunkSize) {
                        chunks.push(files.slice(i, i + uploadChunkSize));
                    }
...
if (modal && modal.classList.contains('hidden')) {
    modal.classList.remove('hidden');
        ...
        fetch(<?php echo json_encode(route('cbt-ops.test-watching.generate-crc')) ?>, {
                method: 'POST',
                headers: {
...
function openCollectDataModal(filter = 'all') {
    ...
    function controlAll(action) {
        ...
        fetch(<?php echo json_encode(route('cbt-ops.test-watching.timer-control-room')) ?>, {
                method: 'POST',
                headers: {
    ...
    function controlTimer(action, userId) {
        ...
        fetch(<?php echo json_encode(route('cbt-ops.test-watching.timer-control')) ?>, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-Requested-With': 'XMLHttpRequest',
...
const testType = <?php echo json_encode($current_test_type) ?>;
            ...
            window.syncTimersFromDatabase = function syncTimersFromDatabase() {
                ...
                return fetch(`{{ route('cbt-ops.test-watching.monitoring') }}?${params.toString()}`, {
...
if (!isFinished) {
        ...
        fetch(`{{ route('cbt-ops.test-watching.monitoring') }}?${params.toString()}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'text/html'
            }
...
function generateBeritaAcaraPDF() {
    ...
    if (BA_TOEIC_LOGO_BASE64) {
        try {
            ...
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(24);
            doc.text('*toeicÂ®', headerLogoX, y + 8);
        }
    } else {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(24);
    ...
    function saveAndUploadBeritaAcara() {
        ...
        try {
            ...
            .then(res => {
                if (res.success) {
                    showToast('âœ… Berita Acara berhasil disimpan & diupload ke FTP Filing System!');
