...
<div class="mx-auto max-w-4xl space-y-6">
    ...
    @if ($mode !== 'loan')
        <form method="POST" action="{{ route('cooperative.manual-loans.adjustment') }}" id="adjustmentForm" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div><label for="adjustment_loan" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Loan Manual</label><select id="adjustment_loan" name="loan_rec_id" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm"><option value="">-- Pilih Loan Manual --</option>@foreach ($loans as $loan)<option value="{{ $loan->rec_id }}">{{ $loan->trnno }} | {{ $loan->member?->icuno ?? '-' }} | {{ $loan->member?->icunm ?? $loan->descr }} | {{ $loan->trndt ? date('d/m/Y', strtotime($loan->trndt)) : '-' }} | Rp {{ number_format((int) $loan->totalloan, 0, ',', '.') }} | {{ $loan->isSettledIndicative() ? 'Lunas' : 'Berjalan' }}</option>@endforeach</select><p class="mt-1 text-[11px] text-gray-400">Format: nomor loan | nomor anggota | nama | tanggal | pokok | status.</p></div>
            <div id="currentSchedule" class="hidden overflow-hidden rounded-xl border border-blue-100 bg-blue-50/40 sm:col-span-2"><div class="border-b border-blue-100 px-4 py-3"><p class="text-xs font-bold uppercase tracking-wide text-blue-700">Jadwal Loan Terpilih</p><p id="currentScheduleMeta" class="mt-1 text-xs text-blue-600"></p></div><div class="max-h-64 overflow-auto bg-white"><table class="w-full min-w-[620px] text-left text-xs"><thead class="sticky top-0 bg-gray-50 uppercase text-gray-500"><tr><th class="px-3 py-2">#</th><th class="px-3 py-2">Periode</th><th class="px-3 py-2 text-right">Pokok</th><th class="px-3 py-2 text-right">Bunga</th><th class="px-3 py-2 text-right">Total</th><th class="px-3 py-2 text-right">Sisa Pokok</th></tr></thead><tbody id="currentScheduleRows" class="divide-y divide-gray-100"></tbody></table></div></div>
    ...
    <form method="POST" action="{{ route('cooperative.manual-loans.store') }}" id="manualLoanForm" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div><label for="trndt" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Tanggal Pinjaman</label><input id="trndt" name="trndt" type="date" value="{{ old('trndt', date('Y-m-d')) }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm"></div>
            ...
            <div><label for="calculation_method" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Metode</label><select id="calculation_method" name="calculation_method" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm"><option value="flat">Flat</option><option value="effective">Efektif</option><option value="annuity">Anuitas</option></select></div>
            <div><label for="startper_display" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Periode Mulai</label><input type="hidden" id="startper" name="startper"><div id="startper_display" class="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-bold text-gray-800">Otomatis dari tanggal pinjaman</div><p class="mt-1 text-[11px] text-gray-400">Siklus tutup buku: tanggal 21 masuk periode bulan berikutnya.</p></div>
...
(function () {
    ...
    memberSelect.addEventListener('change', () => { memberName.value = memberSelect.selectedOptions[0]?.dataset.name || ''; save.disabled = true; });
    if (window.jQuery && jQuery.fn.select2) {
        jQuery(memberSelect).select2({ placeholder: 'Cari nomor atau nama anggota...', allowClear: true, width: '100%' });
        jQuery(memberSelect).on('change', () => { memberName.value = memberSelect.selectedOptions[0]?.dataset.name || ''; save.disabled = true; });
    }
    updatePeriod();
