...
@section('title', 'RUN-ITC | Verifikasi OTP Sinkron Akun')
...
<div class="flex min-h-screen items-center justify-center bg-gray-50 p-4">
    <div class="relative w-full max-w-md overflow-hidden rounded-3xl border border-gray-100 bg-white p-10 shadow-xl">
        ...
        <div class="mb-8 text-center">
            ...
            <p class="text-sm text-gray-500">Masukkan 5 digit kode OTP yang telah dikirim ke email pemilik akun <strong>{{ $target_account }}</strong>.</p>

            <div class="mt-4 inline-block rounded-full border border-red-100 bg-red-50 px-4 py-1.5 text-sm font-semibold text-red-600">
                <i class="far fa-clock mr-1"></i> Sisa Waktu: <span id="countdown">{{ $expires_at }}</span>
            </div>
        </div>

        @if (session('success'))
            <div class="mb-5 rounded-md border border-green-200 bg-green-50 px-4 py-3 text-sm font-semibold text-green-700">{!! session('success') !!}</div>
        @endif
        ...
        <form method="POST" action="{{ route('cooperative.members.sync.verify.store', ['ref_token' => $ref_token]) }}" class="w-full">
            ...
            <button type="submit"
                    class="mb-6 flex w-full items-center justify-center rounded-xl bg-indigo-600 px-4 py-3.5 font-semibold text-white shadow-md transition-colors hover:bg-indigo-700 hover:shadow-lg">
                Verifikasi Sekarang
            </button>
        </form>
