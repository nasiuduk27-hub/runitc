...
@section('title', 'RUN-ITC | Tambah Transaksi Bank')
...
<div class="mx-auto max-w-4xl space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Tambah Transaksi Bank</h1>
            <p class="mt-0.5 text-sm text-gray-500">Pilih nomor referensi dari icu_mtrx2hrd, amount terisi otomatis namun tetap bisa disesuaikan.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('cooperative.bank-transactions.store') }}" class="space-y-5">
        @csrf
        <div class="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="number" id="amount" name="amount" min="1" step="1" value="{{ old('amount') }}"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <label class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-600 transition has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50 has-[:checked]:text-blue-700">
                            <input type="radio" name="dbocr" value="D" {{ old('dbocr') === 'D' ? 'checked' : '' }} class="h-4 w-4 accent-brand-primary" required>
                            Debit
                        </label>
                        <label class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-600 transition has-[:checked]:border-green-600 has-[:checked]:bg-green-50 has-[:checked]:text-green-700">
                            <input type="radio" name="dbocr" value="C" {{ old('dbocr', 'C') === 'C' ? 'checked' : '' }} class="h-4 w-4 accent-brand-primary" required>
                            Kredit
                        </label>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="trndt" name="trndt" value="{{ old('trndt', $defaultTrndt) }}"
                           class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 outline-none transition focus:border-brand-primary focus:bg-white focus:ring-2 focus:ring-brand-primary/20" required>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" id="descr" name="descr" maxlength="100" value="{{ old('descr') }}"
            ...
            <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 text-xs text-gray-500">
                Disimpan ke tabel legacy <span class="font-bold text-gray-700">icu_bank_trx</span> | periode <span class="font-bold text-gray-700">{{ $defaultPprdk }}</span> |
                referensi divalidasi terhadap <span class="font-bold text-gray-700">icu_mtrx2hrd</span>.
            </div>

            <div class="flex flex-wrap gap-2">
                <button type="submit"
                        onclick="return confirm('Simpan transaksi bank ke sistem lama?')"
        ...
        function applyReference() {
            ...
            if (amount !== undefined && amount !== '') {
                ...
                $refInfo.removeClass('hidden').html(
                    ...
                    (option.data('cmpcd') ? ' &middot; Company: ' + option.data('cmpcd') : '') +
