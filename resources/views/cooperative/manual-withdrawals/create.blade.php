...
<div class="mx-auto max-w-3xl space-y-6">
    ...
    <form method="POST" action="{{ route('cooperative.manual-withdraw.store') }}" id="manualWithdrawForm" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        @csrf
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div><label for="amount" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Nominal Penarikan</label><input id="amount" name="amount" type="text" inputmode="numeric" autocomplete="off" value="{{ old('amount') }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm"></div>
            <div><label for="bank_code" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Bank Tujuan (opsional)</label><select id="bank_code" name="bank_code" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm"><option value="">-</option>@foreach ($bankOptions as $code => $label)<option value="{{ $code }}" @selected(old('bank_code') == $code)>{{ $label }}</option>@endforeach</select></div>
            <div><label for="account_name" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Nama Rekening</label><input id="account_name" name="account_name" value="{{ old('account_name') }}" maxlength="150" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm"></div>
            <div><label for="account_no" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Nomor Rekening</label><input id="account_no" name="account_no" value="{{ old('account_no') }}" maxlength="80" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm"><p class="mt-1 text-[11px] text-gray-400">Jika salah satu data rekening diisi, semuanya wajib lengkap.</p></div>
            <div class="sm:col-span-2"><label for="reason" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-500">Alasan (opsional)</label><textarea id="reason" name="reason" rows="2" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm">{{ old('reason') }}</textarea></div>
        </div>
        <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-brand-primary px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-primaryHover"><i class="fas fa-save"></i> Simpan Withdraw Manual</button>
    </form>
</div>
@endsection
