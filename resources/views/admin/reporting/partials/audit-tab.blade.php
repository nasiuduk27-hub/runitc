<div class="tab-content {{ $type === 'user_activity' ? 'hidden' : '' }}" id="tab_{{ $type }}">
    <div class="space-y-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div class="grid grid-cols-1 items-end gap-3 sm:grid-cols-4">
            <div><label class="mb-1 block text-xs font-semibold text-gray-600">Date From</label><x-date-input name="{{ $prefix }}_date_from" id="{{ $prefix }}_date_from" value="{{ $defaultFrom }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"></div>
            <div><label class="mb-1 block text-xs font-semibold text-gray-600">Date To</label><x-date-input name="{{ $prefix }}_date_to" id="{{ $prefix }}_date_to" value="{{ $defaultTo }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"></div>
