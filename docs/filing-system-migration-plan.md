...
| `share_cat`  | Kategori share 1='18' ; 2='CID' ; 3='ITC' ; 4='ALL' |
...
Nomor file akan dibuat menggunakan tabel `sysitc_serialno` dengan mengikuti pola function `GetSernoWeb`.
...
## Aturan Tampilan Folder (Model A)

Setiap folder di sidebar punya aturan tampil tersendiri. Prinsip Model A:
**menu siaran (Share With Me / Divisi / Company) tidak menampilkan file milik user sendiri**
agar tidak terjadi duplikasi dengan menu My Drive.

| Menu             | Kondisi tampil                                                                                                             |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------- |
| My Drive         | `file_system.userid = user` dan status `active` / `archived`                                                              |
| Share With Me    | `userid <> user`, status `active`, ada `file_shareto` `share_cat=1` dengan `othercode = user`                              |
| Divisi/Departemen| `userid <> user`, status `active`, ada `file_shareto` `share_cat=2` dengan `othercode` = kode divisi user                   |
| Company / Staff  | `userid <> user`, status `active`, ada `file_shareto` `share_cat=3` (company user) ATAU `share_cat=4` (semua user)          |
| Sampah           | `userid = user` dan status `trashed`                                                                                       |

Kode divisi user diambil dari resolusi grup akses (`sysitc_usracc` + `sysitc_grpacc` -> `grpdesc`),
dipetakan ke master division `sys_msttable tbl_code='55'` (lihat `resolveDepartmentCodes` di
`FilingPermissionService`).

### Kode Divisi LOG

Divisi Logistik memakai kode `LOG` pada `sys_msttable tbl_code='55'`:
...
Grup akses yang mengandung kata `logistic` / `logistik` / `warehouse` dipetakan ke `LOG`.
Sebelumnya grup tersebut dipetakan ke `WLD` (Warehousing & Logistics); dengan adanya divisi
`LOG` tersendiri, pemetaan diarahkan ke `LOG`.

## Auto Expired & Auto Purge Sampah (Cron)

`filing:process-expired` (via `tools/process_expired_filing_files.php` atau `php artisan filing:process-expired`)
berjalan otomatis setiap hari pukul 03:00 (didaftarkan di `bootstrap/app.php` → `withSchedule`).

Service `FilingExpiryService` bersifat table-aware:
- Saat `FILING_USE_NEW_TABLES=true` → bekerja pada tabel `file_system`.
- Saat `false` → bekerja pada tabel legacy `sys_filing`.

Alur:
1. **Expired** — file `status='active'` dengan `expired_at` sudah lewat dan `expired_processed_at` kosong
   akan diproses sesuai `expired_action` (`none` → `expired`, `archive` → `archived`, `trash` → `trashed`).
   Aksi `delete` untuk MVP di-fallback ke `trash` (auto-hard-delete dinonaktifkan).
2. **Auto purge sampah** — file `status='trashed'` dengan `deleted_at` lebih dari
   `FILING_TRASH_RETENTION_DAYS` (default 30) akan dihapus fisik dari storage (FTP)
   lalu ditandai `status='deleted'`. Jika path fisik kosong / file sudah tidak ada, DB tetap ditandai `deleted`.

Untuk tabel baru, path fisik dibentuk dari `folder_loc` + `sys_msttable.notes` + `upload_flnm`
(melalui `FileSystemDrive::buildStoragePath`).

Catatan server: pastikan crontab menjalankan scheduler Laravel setiap menit agar schedule di atas aktif:
