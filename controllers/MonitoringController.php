...
class MonitoringController
{
    ...
    private function getAttendanceBaseItems(int $adminRecId, string $adminNo, string $testType): array
    {
        ...
        foreach ($this->monitoringModel->getAttendanceParticipantsByAdmin($adminRecId) as $row) {
            ...
            $items[$auth] = [
                ...
                'authcode' => $auth,
                'name' => deccrypt(trim((string) ($row['regnm'] ?? '')), $psyskunci) ?: '-',
                'dob' => deccrypt(trim((string) ($row['dob'] ?? '')), $psyskunci) ?: '-',
                'idno' => deccrypt(trim((string) ($row['idno'] ?? '')), $psyskunci) ?: '-',
                'hpno' => deccrypt(trim((string) ($row['hpno'] ?? '')), $psyskunci) ?: '-',
                'attendance' => $this->attendanceDefaultStatus((string) ($row['statrec'] ?? '')),
                'note' => '',
                'groupcd' => deccrypt(trim((string) ($row['groupcd'] ?? '')), $psyskunci) ?: '-',
                'custom1' => deccrypt(trim((string) ($row['custom1'] ?? '')), $psyskunci) ?: '',
                'custom2' => deccrypt(trim((string) ($row['custom2'] ?? '')), $psyskunci) ?: '',
                'custom3' => deccrypt(trim((string) ($row['custom3'] ?? '')), $psyskunci) ?: '',
            ];
        }
    ...
    private function canCollectCrcFromRoom(int $adminRecId, int $subAdminId): bool
    {
        ...
        if (!$spvData) {
            return userHasTadRole($this->pdoRun, $userId, ['SUPER ADMIN']);
        }
    ...
    private function getPageData(): array
    {
        ...
        try {
            ...
            $isAdminStaff = userHasTadRole($this->pdoRun, $itc_id, ['TAD ADMIN', 'TAD STAFF', 'SUPER ADMIN']);
            if ($isAdminStaff) {
