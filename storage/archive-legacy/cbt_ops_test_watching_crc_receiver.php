 ...
 *   CRC Individual: curl -X POST ... -F "type=crc_individual" -F "admin_no=B00932" -F "files[]=@A001.CRC" -F "files[]=@A002.CRC"
...
const RCVR_CRC_INDIVIDUAL_MAX_FILES = 500;
const RCVR_CRC_INDIVIDUAL_MAX_FILE_SIZE = 1048576;
const RCVR_CRC_INDIVIDUAL_MAX_TOTAL_SIZE = 10485760;
...
if (! in_array($type, ['crc', 'crc_individual', 'berita_acara', 'filing', 'attendance'], true)) {
    rcvrJsonResponse(400, ['success' => false, 'message' => 'Tipe tidak dikenal. Gunakan "crc", "crc_individual", "berita_acara", "filing", atau "attendance".']);
}
...
if ($type === 'crc_individual') {
    ...
    if (empty($uploads)) {
        rcvrJsonResponse(400, ['success' => false, 'message' => 'File CRC belum dikirim.']);
    }

    if (count($uploads) > RCVR_CRC_INDIVIDUAL_MAX_FILES) {
        rcvrJsonResponse(400, ['success' => false, 'message' => 'Maksimal '.RCVR_CRC_INDIVIDUAL_MAX_FILES.' file .CRC sekali upload.']);
    }

    $totalUploadSize = 0;
    foreach ($uploads as $upload) {
        $totalUploadSize += (int) ($upload['size'] ?? 0);
    }
    if ($totalUploadSize > RCVR_CRC_INDIVIDUAL_MAX_TOTAL_SIZE) {
        rcvrJsonResponse(400, ['success' => false, 'message' => 'Total ukuran file CRC maksimal 10MB sekali upload.']);
    }
    ...
    $errors = [];
    foreach ($uploads as $upload) {
        $origName = basename((string) ($upload['name'] ?? ''));
        $safeName = preg_replace('/\s+/', '_', $origName);
        $safeName = preg_replace('/[^A-Za-z0-9_\-.]/', '_', $safeName);
        $safeName = trim((string) $safeName, '._-');

        if (($upload['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
            $errors[] = ['file_name' => $origName, 'message' => 'Upload gagal. Error: '.($upload['error'] ?? 'unknown')];

            continue;
        }

        if ($safeName === '' || strtolower(pathinfo($safeName, PATHINFO_EXTENSION)) !== 'crc') {
            $errors[] = ['file_name' => $origName, 'message' => 'Format file harus .CRC.'];
        ...
        if ($tmpName === '' || ! is_uploaded_file($tmpName)) {
            $errors[] = ['file_name' => $origName, 'message' => 'File upload tidak valid.'];

            continue;
        }

        $fileSize = filesize($tmpName);
        if ($fileSize === false || $fileSize <= 0 || $fileSize > RCVR_CRC_INDIVIDUAL_MAX_FILE_SIZE) {
            $errors[] = ['file_name' => $origName, 'message' => 'File kosong atau lebih dari 1MB.'];
        ...
        if (! @move_uploaded_file($tmpName, $targetPath)) {
            $errors[] = ['file_name' => $origName, 'message' => 'Gagal menyimpan file.'];

            continue;
        }

        $relativePath = $testDate.'/CRC/'.$safeName;
        $uploaded[] = rcvrFilePayload($adminNo, $relativePath, 'crc_individual', $targetPath);
    }

    rcvrJsonResponse(empty($uploaded) ? 400 : 200, [
        'success' => ! empty($uploaded),
        'message' => count($uploaded).' file CRC individu berhasil disimpan'.(! empty($errors) ? ', '.count($errors).' gagal.' : '.'),
        'type' => $type,
        'admin_no' => $adminNo,
