...
require_once __DIR__.'/app/Legacy/Controllers/AuthController.php';

// Nginx/local dev may route extensionless URLs here because .htaccess is Apache-only.
$requestPath = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH) ?: '/';
$basePath = parse_url(BASE_URL, PHP_URL_PATH) ?: '';
$basePath = '/'.trim($basePath, '/');
$basePath = $basePath === '/' ? '' : $basePath;

if ($basePath !== '' && (stripos($requestPath, $basePath.'/') === 0 || strcasecmp($requestPath, $basePath) === 0)) {
    $requestPath = substr($requestPath, strlen($basePath)) ?: '/';
}

$requestPath = '/'.trim($requestPath, '/');
if ($requestPath !== '/' && strtolower($requestPath) !== '/index') {
    $targetFile = realpath(__DIR__.$requestPath.'.php');
    $baseDir = realpath(__DIR__);

    if ($targetFile && $baseDir && str_starts_with($targetFile, $baseDir.DIRECTORY_SEPARATOR)) {
        require $targetFile;
        exit;
    }
}
...
$controller = new AuthController($pdo_run, $pdo, $pdo_bot); // TANGKAP HASILNYA DI SINI
$data = $controller->handleRequest();

// Pecah datanya supaya bisa dipakai HTML
$error = $data['error'] ?? '';
...
<head>
    ...
    <!-- Sekarang $carousels dan $error sudah dikenali di sini -->
    <?php include __DIR__.'/app/Legacy/Includes/head.php'; ?>
</head>
