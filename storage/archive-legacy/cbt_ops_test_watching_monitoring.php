...
if (file_exists($configPath)) {
    ...
    exit('config.php tidak ditemukan di: '.$configPath);
}

if (file_exists($nisnlibPath)) {
    require_once $nisnlibPath;
} else {
    exit('nisnlib.php tidak ditemukan di: '.$nisnlibPath);
}
...
if (! function_exists('shifting')) {
    exit('Function shifting() belum ter-load. Cek isi file nisnlib.php');
}

if (! function_exists('deccrypt')) {
    exit('Function deccrypt() belum ter-load. Cek isi file nisnlib.php');
}
...
<form action="upload_crc" method="POST" enctype="multipart/form-data" class="mt-4" id="manualCrcUploadForm">
