...
class Auth
{
    ...
    public function authenticate(string $accountId, string $password): array|false
    {
        ...
        if ($this->accountExistsInDb($this->db, $accountId)) {
            return false;
        }
    ...
    private function accountExistsInDb(PDO $pdo, string $accountId): bool
    {
        $stmt = $pdo->prepare('SELECT rec_id FROM sysitc_login WHERE account_id = ? LIMIT 1');
