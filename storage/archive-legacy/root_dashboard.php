...
require_once __DIR__.'/app/Legacy/Includes/menu_guard.php';
require_once __DIR__.'/app/Legacy/Includes/tad_access.php';
require_once __DIR__.'/app/Legacy/Includes/tad_participant_recap.php';
/** @var PDO $pdo_run */
...
require_once __DIR__.'/app/Legacy/Includes/layout_header.php';
?>
        ...
        <div class="widget-card col-span-1 lg:col-span-4 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden relative group" data-widget-id="tad-participant-recap">
            ...
            <div class="px-5 py-4 border-b border-gray-100 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                    ...
                    <h2 class="text-base font-extrabold text-gray-900">Rekap Peserta per Nomor Admin</h2>
                    <p class="text-xs text-gray-500 mt-0.5">Detail nomor admin dengan subtotal per tanggal.</p>
                </div>
                            ...
                            <?php foreach ($tadParticipantRecapRows as $row) { ?>
                                <?php foreach (($row['items'] ?? []) as $index => $item) { ?>
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-5 py-3 text-xs text-gray-600 leading-relaxed align-top"><?= htmlspecialchars($item['item_text'] ?? '-') ?></td>
                                        <td class="px-5 py-3 text-right align-top font-extrabold text-gray-900"><?= number_format((int) ($item['total_participants'] ?? 0)) ?></td>
                                        <td class="px-5 py-3 text-right align-top font-extrabold text-emerald-700"><?= number_format((int) ($item['finished_participants'] ?? 0)) ?></td>
                                    </tr>
                                <?php } ?>
                                <tr class="bg-emerald-50/80 border-t border-emerald-100">
                                    <td class="px-5 py-2 text-xs font-black text-emerald-800 uppercase">Total <?= date('d M Y', strtotime($row['date'])) ?></td>
...
<?php require_once __DIR__.'/app/Legacy/Includes/layout_footer.php'; ?>
