        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-4 space-y-2 text-sm">
              ...
              <?php foreach ($locDist as $loc) {
                  $pct = ($loc['total'] / $maxLoc) * 100; ?>
              <div>
              ...
              <?php } ?>
              <?php if (empty($locDist)) { ?><div class="text-center text-gray-400 py-4 text-xs">Tidak ada data</div><?php } ?>
            </div>
          </div>
          <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100">
                ...
                <tr data-name="<?= strtolower(htmlspecialchars($room['admin_no'])) ?>" class="hover:bg-gray-50">
                  ...
                  <td class="px-3 py-2">
                    <span class="inline-block px-1.5 py-0.5 rounded text-[9px] font-bold
                      <?= match ($room['status']) {
                          'active' => 'bg-green-100 text-green-700','error' => 'bg-red-100 text-red-700','completed' => 'bg-gray-100 text-gray-500','pending' => 'bg-amber-100 text-amber-700',default => 'bg-gray-100 text-gray-500'
                      } ?>">
                      <?= $room['status'] ?>
                    </span>
                  </td>
